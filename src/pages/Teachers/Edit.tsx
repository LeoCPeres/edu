...
export function EditTeacher() {
  ...
  const teacherParam = state.teacher as TeachersProps;

  const { user } = useAuth();

  const [phoneNumber, setPhoneNumber] = useState(teacherParam.whatsapp);
  const [biography, setBiography] = useState(teacherParam.biography);
  const [price, setPrice] = useState<number>(teacherParam.price);
  const [subject, setSubject] = useState(teacherParam.subject);
  ...
  return (
    <Flex direction="column">
      <Flex
        ...
        mt="64px"
      >
        <Flex align="center" gap="24px" flexDir="column">
          <Avatar
            name={user?.name}
            src={user?.avatar}
            w="132px"
          ...
          <Heading fontSize="26px" fontFamily="Archivo" color="white">
            {user?.name}
          </Heading>
          <Text>{}</Text>
        </Flex>
